...
export class CategoriesComponent extends AppComponentBase {
    ...
    exportToExcel(): void {
        this._categoriesServiceProxy.getCategoriesToExcel(this.filterText, this.isActiveFilter).subscribe((result) => {
    ...
    action(event: any, record: any) {
        switch (event) {
            case 'View':
                ...
                break;
            case 'Edit':
                this.createOrEditCategoryModal.show(record.category.id);
                break;
            case 'Delete':
                this.deleteCategory(record.category);
                break;
            case 'History':
