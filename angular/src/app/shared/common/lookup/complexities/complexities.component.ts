...
export class ComplexitiesComponent extends AppComponentBase {
    @ViewChild('createOrEditComplexityModal', { static: true })
    createOrEditComplexityModal: CreateOrEditComplexityModalComponent;
    ...
    deleteComplexity(complexity: ComplexityDto): void {
        this.message.confirm('', this.l('AreYouSure'), (isConfirmed) => {
            if (isConfirmed) {
                this._complexitiesServiceProxy.delete(complexity.id).subscribe(() => {
                    this.reloadPage();
    ...
    exportToExcel(): void {
        this._complexitiesServiceProxy
            .getComplexitiesToExcel(this.filterText, this.isActiveFilter)
            .subscribe((result) => {
    ...
    changeStatus($event, record) {
        this._complexitiesServiceProxy
            .updateComplexityStatus(record.complexity.id, $event.checked)
            .subscribe((val) => {});
    }
}
